...
const baseURL = "https://real-tick-suit.cyclic.app/api/stories";
...
function CommentView(props) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios
        .put(
          `${baseURL}/${storyId}/comments/${commentId}`,
          { comment },
  ...
  React.useEffect(() => {
    axios
      .get(`${baseURL}/${storyId}/comments/${commentId}`)
      .then((response) => {
    ...
    <div className="commentView">
      ...
      <div className="comment-model">
        <div className="editDelGroup">
          ...
          {userName === comment.comment.username ? (
            <span onClick={handleDelete} className="commentBtn" id="delBtn">
              <AiOutlineDelete />
            </span>
          ) : (
            ""
